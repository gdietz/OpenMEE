...
class ResultsWindow(QMainWindow, ui_results_window.Ui_ResultsWindow):
    ...
    def __init__(
        ...
        # text for export not included 'additional values'
        self.texts_for_export = []
        ...
        self.export_btn.clicked.connect(
            lambda: self.export_results(results["results_data"])
        )
        ...
        if "results_data" in results and islistortuple(results["results_data"]) and len(results["results_data"]) > 0:
            if self.show_additional_values:
                ...
                self.add_additional_values_texts(
                    results["results_data"],
                    ADDITIONAL_VALUES_TO_ALWAYS_SHOW,
                    additionalvaluesparent=False,
                )
    ...
    def export_results(self, data):
        ...
        fpath = QFileDialog.getSaveFileName(
            caption="Choose location to save file",
            filter="Text (*.txt)",
            directory=fpath,
        )
        fpath = str(fpath)
        ...
        with open(fpath, 'w') as f:
            ...
            if self.show_additional_values:
                ...
                for key, value_data in data:
                    # write out key and description
                    f.write("%s: %s\n" % (key, value_data['description']))
    ...
    def add_images(self):
        ...
        ungrouped_images = [
            (title, self.images[title]) for title in image_order
        ]
        ordered_images = ungrouped_images

        if self.image_order is None:
            # add to the arguments to make more groups, also make sure to add
            # them in add_text
    ...
    def purified_title_str(self, title):
        '''
        Strips out metadata from title i.e.
        if title is "Forest Plot__phlyo" this strips out the __phylo
        '''
    ...
    def add_additional_values_texts(
        self,
        data,
        keys_to_display=None,
        additionalvaluesparent=True,
        sublist_prefix="__",
    ):
        '''
        ...
        of an 'Additional Values' item in the nav tree.
            keys_to_display: List of keys to always display regardless of
                ...
                keys are displayed.
            additionalvaluesparent: display the additional values under a
                parent item on the left side of the results window
        '''
        ...
        for key, value_data in data:
            ...
            if key[0:len_sublist_prefix] == sublist_prefix:
                ...
                current_parent = self._add_text_item(
                    key[len_sublist_prefix:],
                    val_str,
                    parent_item=additional_values_item,
                )
                # self.items_to_ignore.append(current_parent)
            else:
                self._add_text_item(
                    key+": %s" % value_data['description'],
                    val_str, parent_item=current_parent,
                )

    def _add_text_item(self, title, text, parent_item=None):
        '''
        parent_item is the parent item in the nav tree
        If title is none, no title is created and no reference to the added
        item is returned.
        '''
    ...
    def _group_items(self, items, groups):
        '''
        Groups items together if their title contains an element in a group
        list.

        Paramters:
            items - tuple of key,value pairs i.e. (title,text)
            groups - Each group is a list of strings to which item titles
            should be matched i.e:
                _group_items(items, ['NLR','PLR'], ['sens','spec'])
        '''
    ...
    def create_text_item(self, text, position):
        ...
        # This is a dumb way to escape the text for conversion to html but it
        # works for now
        text = text.replace('\r', '')
        text = text.replace('<', '&lt;')
        text = text.replace('\n', '<br />')
    ...
    def _make_context_menu(
        ...
        def _graphics_item_context_menu(event):
            ...
            def add_edit_plot_menu_action(menu):
                ...
                # in sens and spec plotted on the same canvass) forest plots
                # for now
        ...
        if file_path != "":
            ...
            if plot_type == "forest":
                if self._is_side_by_side_fp(title):
                    python_to_R.generate_forest_plot(
                        file_path,
                        side_by_side=True,
                    )
                else:
                ...
                python_to_R.regenerate_exploratory_plot(
                    params_path,
                    file_path,
                    plot_type=plot_type,
                )
            elif plot_type == "forest plot of coefficients":
                python_to_R.regenerate_forest_plot_of_coefficients(
                    file_path,
                    params_path,
                    fmt,
                )
    ...
    def edit_image(
        ...
        if plot_type == "forest":
            ...
            if edit_form.exec_():
                ...
                python_to_R.regenerate_funnel_plot(
                    params_path,
                    png_path,
                    edited_funnel_params=new_funnel_params,
                )
            ...
            edit_form = EditDataExplorationPlotForm(
                form_type=plot_type,
                params=params,
            )
            if edit_form.exec_():
