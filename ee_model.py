...
# marker placed at start of a label(but is not displayed)
LABEL_PREFIX_MARKER = "*lbl*"
...
DEFAULT_LAST_ANALYSIS_SELECTIONS = {
    'data_locations': {
        MEANS_AND_STD_DEVS: {},
...
class EETableModel(QAbstractTableModel):
    ...
    def __init__(self, undo_stack, model_state=None):
        ...
        # the values in this dict will only be valid if not much messing
        # around has happened
        ...
        if model_state is None:
            ...
            self.dirty = False
            # maps data_types to column choices
        ...
        # state to set/reset while pasting/importing
        self.big_paste_mode = False
    ...
    def set_conf_level(self, conf_level, ignore_undo=False):   # i.e. 95
        if ignore_undo:
            ...
            self.do_when_conf_level_changes(
                old_conf_level,
                conf_level,
                use_undo=False,
            )
            ...
            self.undo_stack.push(GenericUndoCommand(
                redo_fn=redo_fn,
                undo_fn=undo_fn,
    ...
    def do_when_conf_level_changes(
        self,
        old_conf_level,
        new_conf_level,
        use_undo=True,
        ...
        for var_grp in self.variable_groups:
            ...
            # make sure a variable is assigned to each column of the raw
            # effects
            if not var_grp.raw_effects_full():
                continue

            # transform raw data columns to transform scale, then get them back
            # again in no transform scale at the new conf level
            raw_data_location = {
                RAW_EFFECT: var_grp.get_var_with_key(RAW_EFFECT),
            ...
            for effect in [RAW_EFFECT, RAW_LOWER, RAW_UPPER]:
                ...
                if use_undo:
                    ...
                    cmd = GenericUndoCommand(
                        redo_fn=redo_fn,
                        undo_fn=undo_fn,
                        on_redo_exit=partial(
                            self.emit_change_signals_for_col,
                            col=col,
                        ),
                        on_undo_exit=partial(
                            self.emit_change_signals_for_col,
                            col=col,
                        ),
                    )
...
# ##################### Analysis parameters section start ####################
    ...
    def update_data_location_choices(self, data_type, data_locations):
        ...
        self.last_analysis_selections[
            'data_locations'
        ][data_type].update(data_locations)
        print("Data location choices are now: %s" % str(
            self.last_analysis_selections['data_locations']
        ))
    ...
    def update_previously_included_studies(self, included_studies):
        '''
        Included studies is a set() of studies obtained from the meta-analysis
        wizard or at least should be, not a dictionary study -> boolean
        '''
    ...
    def try_to_get_last_selection(self, key):
        ...
        if it fails, reset the self.last_analysiss selections for the key to
        the default and return the result. This functions helps ensure
        backwards compatibility with old *.ome pickles
        '''
    ...
    def update_selected_cov_and_covs_to_values(
        self,
        selected_cov,
        covs_to_values,
    ):
    ...
    def get_last_analysis_selections(self):
        '''
        All the parameters previously selected in the course of choosing
        options on wizard screens
        '''
        return self.last_analysis_selections
    ...
    def get_previous_selected_cov_and_covs_to_values(self):
        return [
            self.last_analysis_selections['selected_cov'],
            self.last_analysis_selections['covs_to_values'],
        ]
    ...
    def get_data_type_from_data_location(self, data_location):
        ...
        var = self.model.get_variable_assigned_to_column(
    ...
    def get_studies_in_current_order(self):
        studies = [
            self.rows_2_studies[row] for row in sorted(self.rows_2_studies.keys())
        ]
        return studies

    def __str__(self):
        rows_2_study_ids = dict([
            (row, study.get_id()) for (row, study) in self.rows_2_studies.items()
        ])
    ...
    def get_continuous_columns(self):
        '''
        Returns a list of column indices with variables that have continuous
        data
        '''
    ...
    def get_trans_effect_columns(self):  # of type TRANS_EFFECT
        ...
        trans_eff_columns = [
            col for col in continuous_columns if is_effect_col(col)
        ]
    ...
    def get_trans_var_columns(self):  # of type TRANS_VAR
        ...
        trans_var_columns = [
            col for col in continuous_columns if is_trans_var_col(col)
        ]
    ...
    def _get_columns_of_type(self, var_type):
        ...
        return sorted(
            [col for col, var in self.cols_2_vars.items() if var.get_type() == var_type]
        )

    def get_variables(self, var_type=None):
        '''
        Gets all the variables in a list, or all the variables of a particular
        type
        '''
        ...
        variables = self.dataset.get_variables()
        desired_variables = [
            var for var in variables if var.get_type() == var_type
        ]
    ...
    def emit_change_signals_for_col(
        self,
        col,
        header_only=False,
        data_only=False,
    ...
    def emit_change_signals_from_start_to_end(
        self,
        srow,
        scol,
        erow,
        ...
        # row, scol upper left row, column
        ...
        print("emitting change signals from (%d, %d) to (%d, %d)" % (
            srow,
            scol,
            erow,
            ecol,
...
#                self.col_2_max_length[col] = max(col_content_lengths)
    ...
    def shift_column_assignments(self, columns, shift_amount):
        ...
        self.cols_2_vars = dict([
            (shift_col(col), var) for col, var in self.cols_2_vars.items()
        ])
    ...
    def shift_row_assignments(self, rows, shift_amount):
        ...
        self.rows_2_studies = dict([
            (shift_row(row), study) for row, study in self.rows_2_studies.items()
        ])
    ...
    def mark_column_as_label(
        self,
        column_index,
        overridden_studies_2_labels={},
    ):
        '''
        Sets the contents of this column as the labels for the studies in the
        column
        '''
        ...
        # Delete references to that column in the studies and remove it from
    ...
    def remove_variable(self, var):
        '''
        Deletes a variable from the collection and removes it from the mapping
        of columns to variables
        '''
    ...
    def removeColumns(self, column, count=1, index=QModelIndex()):
        ...
        self.undo_stack.push(RemoveColumnsCommand(
            model=self,
            column=column,
            count=count,
    ...
    def removeRows(self, row, count=1, index=QModelIndex()):
        ...
        self.undo_stack.push(RemoveRowsCommand(
            model=self,
            row=row,
            count=count,
    ...
    def insertRows(self, row, count=1, index=QModelIndex()):
        ...
        self.undo_stack.push(InsertRowsCommand(
            model=self,
            row=row,
            count=count,
    ...
    def insertColumns(self, column, count=1, index=QModelIndex()):
        ...
        self.undo_stack.push(InsertColumnsCommand(
            model=self,
            column=column,
            count=count,
    ...
    def make_new_variable(self, label=None, var_type=CATEGORICAL):
        new_var = self.dataset.make_new_variable(
            label=label,
            var_type=var_type,
        )
    ...
    def unmark_column_as_label(
        ...
        Parameters:
            column_index
            original_study_labels - mapping of those studies whose labels were
                                    changed to their original values
        '''
        ...
        new_var = self.make_new_variable(
            label=variable_name,
            var_type=CATEGORICAL,
        )
        ...
        for study in studies:
            ...
            # converts the value from a string to a string (overkill but
            # whatever)
            value = self.dataset.convert_var_value_to_type(
                CATEGORICAL,
                CATEGORICAL,
                value,
            )
    ...
    def _var_value_for_display(self, value, var_type):
        ...
        return self.dataset.convert_var_value_to_type(
            old_type,
            CATEGORICAL,
            value,
        ...
        if role == Qt.DisplayRole or role == Qt.EditRole:
            ...
            if is_label_col:
                ...
                return QVariant(QString(
                    self._var_value_for_display(var_value, var.get_type())
                ))
            ...
            if is_label_col:
                ...
                if role == Qt.BackgroundRole:
                    if self._is_empty(
                        var,
                        study=self.rows_2_studies[row] if is_study_row else None,
                    ):
    ...
    def _make_settings_key_for_var_color(
        self,
        var_type,
        var_subtype=None,
        fg=True,
        ...
        '''
        Generates a key to use for accessing the key for the color in the
        QSettings object note: fg means 'foreground'
        '''

        sep = "/"  # built in to QSettings
        var_type_str = VARIABLE_TYPE_STRING_LC[var_type]
        placement = "fg" if fg else "bg"
        # need to check explicitly b/c var_subtype can be 0, a valid option
    ...
    def get_variable_color(self, variable, role=Qt.ForegroundRole):
        ...
        if role == Qt.ForegroundRole:
            color = get_setting(
                self._make_settings_key_for_var_color(
                    var_type,
                    var_subtype,
                    fg=True,
                ),
    ...
    def setData(self, index, value, role=Qt.EditRole, basic_value=False):
        '''
        Parameters:
            index
            value
            role
            basic_value - return a regular python type not a QVariant
        '''
        ...
        if basic_value:
            ...
            # value is a QVariant
            # fix for issue #78
            value_as_string = str(value.toString()).strip() if value not in [
                "",
                None,
                QVariant(),
                QVariant(None),
        ...
        if not self.big_paste_mode:
            self.undo_stack.beginMacro(QString(
                "SetDataMacro: (%d, %d), value: '%s'" % (
                    row,
                    col,
                    value_as_string,
                )
            ))
            self.undo_stack.push(EmitDataChangedCommand(
                model=self,
                index=index,
            ))
        ...
        if is_label_col:
            ...
            if label_valid():
                if not self.big_paste_mode:
                    self.undo_stack.push(SetStudyLabelCommand(
                        study=study,
                        new_label=proposed_label,
                    ))
                    self.adjust_max_length_for_col(
                        col,
                        existing_label,
                        proposed_label,
                    )
            ...
            if make_new_variable:
                # make a new variable and give it the default column header
                # name
                if value_blank:
                    if not self.big_paste_mode:
                        cancel_macro_creation_and_revert_state(self.undo_stack)
                ...
                if not self.big_paste_mode:
                    self.undo_stack.push(MakeNewVariableCommand(
                        model=self,
                        var_name=new_var_name,
                        col=col,
                    ))
                else:
                    self.make_variable_at_column(
                        col=col,
                        var_name=new_var_name,
                    )
            ...
            can_convert_info = self.dataset.can_convert_var_value_to_type(
                var_type,
                value_as_string,
            )
            ...
            formatted_value = self._convert_input_value_to_correct_type_for_assignment(
                value_as_string,
                var_type,
            )
            ...
            if not self.big_paste_mode:
                ...
                self.undo_stack.push(SetVariableValueCommand(
                    study=study,
                    variable=var,
            ...
            # Check to see if variable belongs to a variable group (with effect
            # sizes calculated)
        ...
        if study.is_totally_blank():
            self.undo_stack.push(RemoveStudyCommand(
                model=self,
                row=row,
            ))
    ...
    def test_value_for_setData(self, index, value):
        ...
        row_has_study = row in self.rows_2_studies
        is_label_col = col == self.label_column
        value_as_string = str(value.toString()) if value not in [
            "", None, QVariant(), QVariant(None),
        ] else ""
    ...
    def handle_grouped_variable_from_setData(self, study, var, value):
        ...
        for var_grp in var_grps:
            ...
            if not var_grp.key_in_data(var_key):
                # var is an effect so we don't care
                continue
            ...
            if not are_valid_numbers(
                [study.get_var(v) for v in data_location_as_vars.values()],
            ):
                # quit if not all the required boxes are filled in properly
                continue
            data = python_to_R.gather_data_for_single_study(
                data_location_as_vars,
                study,
            )
            # convert singletons to lists to be suitable for
            # python_to_R.effect_size
            data = dict([(k, [v, ]) for k, v in data.items()])
            # keys: 'yi', 'vi'
            trans_scale_effect_sizes = python_to_R.effect_size(
                metric=var_grp.get_metric(),
            ...
            trans_scale_data = {
                TRANS_EFFECT: trans_scale_effect_sizes['yi'],
            ...
            self.set_single_row_effect_data(
                var_grp,
                study,
                trans_scale_data,
                raw_effect_sizes,
    ...
    def set_single_row_effect_data(
        self,
        var_grp,
        study,
        trans_scale_data,
    ...
    def _convert_input_value_to_correct_type_for_assignment(
        self,
        value,
        var_type,
    ):
    ...
    def add_transformed_effect_sizes_to_model(
        self,
        metric,
        effect_sizes,
        transform_direction,
        ...
        if transform_direction == TRANS_TO_RAW:
            ...
            # Make new variables to hold the results of the effect size
            # calculations
            add_raw_yi_cmd = MakeNewVariableCommand(
                model=self,
                var_name=METRIC_TEXT_SHORT_RAW_SCALE[metric],
                col=raw_yi_col,
                var_type=CONTINUOUS,
            )
            add_raw_lb_cmd = MakeNewVariableCommand(
                model=self,
                var_name="lb(%s)" % METRIC_TEXT_SHORT_RAW_SCALE[metric],
                col=raw_lb_col,
                var_type=CONTINUOUS,
            )
            add_raw_ub_cmd = MakeNewVariableCommand(
                model=self,
            ...
            add_trans_yi_cmd = MakeNewVariableCommand(
                model=self,
                var_name=METRIC_TEXT_SHORT[metric],
                col=trans_yi_col,
                var_type=CONTINUOUS,
            )
            add_trans_vi_cmd = MakeNewVariableCommand(
                model=self,
        ...
        if transform_direction == RAW_TO_TRANS:
            ...
            self.undo_stack.push(SetVariableSubTypeCommand(
                model=self,
                variable=trans_yi,
                new_subtype=TRANS_EFFECT,
            ))
            self.undo_stack.push(SetVariableSubTypeCommand(
                model=self,
                variable=trans_vi,
                new_subtype=TRANS_VAR,
            ))
            ...
            self.undo_stack.push(SetVariableSubTypeCommand(
                model=self,
                variable=raw_yi,
                new_subtype=RAW_EFFECT,
            ))
            self.undo_stack.push(SetVariableSubTypeCommand(
                model=self,
                variable=raw_lb,
                new_subtype=RAW_LOWER,
            ))
            self.undo_stack.push(SetVariableSubTypeCommand(
                model=self,
                variable=raw_ub,
                new_subtype=RAW_UPPER,
            ))
        ...
        self.undo_stack.push(GenericUndoCommand(
            redo_fn=partial(
                self.add_vars_to_col_group, column_group,
                keys_to_vars,
            ),
            undo_fn=partial(
                self.remove_vars_from_col_group,
                column_group,
        ...
        if transform_direction == TRANS_TO_RAW:
            for study, yi_val, lb_val, ub_val in zip(
                studies,
                effect_sizes[RAW_EFFECT],
                effect_sizes[RAW_LOWER],
                effect_sizes[RAW_UPPER],
            ):
                self.undo_stack.push(SetVariableValueCommand(
                    study,
                    raw_yi,
            ...
            for study, yi_val, vi_val in zip(
                studies,
                effect_sizes[TRANS_EFFECT],
                effect_sizes[TRANS_VAR],
            ):
                self.undo_stack.push(SetVariableValueCommand(
                    study,
                    trans_yi,
    ...
    def add_effect_sizes_to_model(
        self,
        metric,
        effect_sizes,
        cols_to_overwrite=None,
    ):

        studies = self.get_studies_in_current_order()
        if not cols_to_overwrite:
            last_occupied_col = sorted(
                self.cols_2_vars.keys(),
                reverse=True,
            )[0]
        ...
        if not bool(cols_to_overwrite):
            self.undo_stack.beginMacro(
                QString("Adding effect size to spreadsheet")
            )
        else:
            self.undo_stack.beginMacro(
                QString("Adding effect size to spreadsheet (overwriting existing columns)")
            )
        ...
        if not bool(cols_to_overwrite):
            ...
            add_vi_cmd = MakeNewVariableCommand(
                model=self,
                var_name=METRIC_TEXT_SHORT[metric],
                col=yi_col,
                var_type=CONTINUOUS,
            )
            add_yi_cmd = MakeNewVariableCommand(
                model=self,
                var_name="Var(%s)" % METRIC_TEXT_SHORT[metric],
                col=vi_col,
                var_type=CONTINUOUS,
        ...
        self.undo_stack.push(SetVariableSubTypeCommand(
            model=self,
            variable=variable_yi,
            new_subtype=TRANS_EFFECT,
        ))
        self.undo_stack.push(SetVariableSubTypeCommand(
            model=self,
            variable=variable_vi,
            new_subtype=TRANS_VAR,
        ))
        ...
        if old_col_group:
            # this will exist in the case of overwriting existing columns
            # probably (unless the columns were constucted by hand)
            col_group = old_col_group
        else:
            col_group = self.make_new_variable_group(
                metric=metric,
                name=METRIC_TEXT_SIMPLE[metric] + " column group",
            )
        keys_to_vars = {
            TRANS_EFFECT: variable_yi,
            TRANS_VAR: variable_vi,
        }
        self.undo_stack.push(GenericUndoCommand(
            redo_fn=partial(
                self.add_vars_to_col_group,
                col_group,
                keys_to_vars,
            ),
            undo_fn=partial(
        ...
        self.set_values_of_variables(
            studies=studies,
            var_yi=variable_yi,
            var_vi=variable_vi,
            raw_yi=effect_sizes['yi'],
            raw_vi=effect_sizes['vi'],
        )

        end_cmd = GenericUndoCommand(
            redo_fn=emit_change_signals,
            undo_fn=do_nothing,
        )
        self.undo_stack.push(end_cmd)
        self.undo_stack.endMacro()
    ...
    def make_new_variable_group(self, metric, name):
        ...
        make_col_grp_cmd = GenericUndoCommand(
            redo_fn=redo_fn,
            undo_fn=undo_fn,
    ...
    def remove_variable_group(self, var_group):
        ...
        remove_var_group_cmd = GenericUndoCommand(
            redo_fn=redo_fn,
            undo_fn=undo_fn,
    ...
    def sort_by_column(self, col):
        ...
        if is_label_column:
            sorted_studies = sorted(
                studies,
                key=lambda study: study.get_label(),
            )
        else:
            ...
            sorted_studies = sorted(
                studies,
                key=lambda study: study.get_var(var),
            )
...
class VariableGroup:
    # Stores info on related groups of variables i.e. effect size and variance,
    # etc
    ...
    def __str__(self):
        ...
        raw_scale_str = "Raw Scale: effect [lower, upper]: %s [%s, %s]" % (
            raw_yi_lbl,
            raw_lb_lbl,
            raw_ub_lbl,
        )
        trans_scale_str = "Transformed Scale: effect, variance: %s, %s" % (
            trans_yi_lbl,
            trans_vi_lbl,
        )
...
class MakeStudyCommand(QUndoCommand):
    ...
    def redo(self):
        if DEBUG_MODE:
            print("redo: make new study_command")
        model = self.model
...
class SetStudyLabelCommand(QUndoCommand):
    ...
    def __init__(self, study, new_label):
        ...
        self.setText(QString("set study label from '%s' to '%s" % (
            self.old_label,
            self.new_label,
        )))
    ...
    def redo(self):
        if DEBUG_MODE:
            print("redo: set study label")
        self.study.set_label(self.new_label)

    def undo(self):
        if DEBUG_MODE:
            print("undo: set study label")
...
class MakeNewVariableCommand(QUndoCommand):
    ...
    def redo(self):
        if DEBUG_MODE:
            print("redo: make new variable_command")
        if self.var is None:
            self.var = self.model.make_new_variable(
                label=self.var_name,
...
class SetVariableValueCommand(QUndoCommand):
    ...
    def __init__(self, study, variable, value):
        ...
        self.setText(QString("Set '%s' from '%s' to '%s' for a study" % (
    ...
    def redo(self):
        if DEBUG_MODE:
            print("redo: set variable value command")
        self.study.set_var(self.variable, self.new_value)

    def undo(self):
        if DEBUG_MODE:
            print("undo: set variable value command")
...
class SetVariableSubTypeCommand(QUndoCommand):
    ...
    def redo(self):
        if DEBUG_MODE:
            print("redo: set variable subtype command")
    ...
    def undo(self):
        if DEBUG_MODE:
            print("undo: set variable subtype command")
...
class RemoveColumnsCommand(QUndoCommand):
    ...
    def __init__(self, model, column, count, description="Remove columns"):
        ...
        # mapping of studies to study labels
        self.studies_2_labels = {}
        # dictionary of variables and their mappings to columns that have been
        # removed
        self.removed_cols_2_vars = {}
        ...
        for col in range(self.column, self.column+self.count):
            ...
            if self.model.column_assigned_to_variable(col):
                # variable index, variable data in studies
                self._store_variable_column_data(col)
    ...
    def undo(self):
        ...
        # shift columns to the right (reclaiming the space for the deleted
        # columns)
...
class InsertRowsCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        self.rows_to_shift_down = [
            row for row in self.model.rows_2_studies.keys() if row >= self.row
        ]
...
class InsertColumnsCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        self.variable_columns_to_shift_right = [
            col for col in self.model.cols_2_vars.keys() if col >= self.column
        ]

        self.model.shift_column_assignments(
            self.variable_columns_to_shift_right,
            self.count,
        )
    ...
    def undo(self):
        ...
        cols_to_shift_left = [
            col+self.count for col in self.variable_columns_to_shift_right
        ]
...
class ChangeVariableFormatCommand(QUndoCommand):
    ...
    def __init__(
        ...
        target_type,
        precision,
        description="Change variable format",
    ):
        ...
        # dictionary mapping studies to their original values for the given
        # variable
        self.orignal_vals = {}

    def redo(self):
        self._store_original_data_values_for_variable()
        self.model.change_variable_type(
            self.variable,
            self.target_type,
            self.precision,
        )
    ...
    def undo(self):
        ...
        if self.key_var_group:
            self.original_variable_group.set_var_with_key(
                key=self.key_var_group,
                var=self.variable,
            )
