...
context("Subgroup Meta-Analysis")

# prepare data
tmp_obj <- new('ContinuousData',
  ...
  covariates=list(new('CovariateValues',
    cov.name='STATE',
...
test_that("Subgroup Meta-Analysis Runs", {
  set.global.conf.level(95)
  results <- subgroup.ma.continuous(fname, tmp_obj, params)

  image.path <- results$images[['Subgroups Forest Plot']]
