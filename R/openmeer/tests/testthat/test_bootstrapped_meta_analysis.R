...
context("Bootstrapped Meta-Analysis")
...
params <- data.frame(
  ...
  digits = 3L,
  fp_col2_str = "[default]",
  num.bootstrap.replicates = 10L,
  fp_col4_str = "Ev/Ctrl"
)

# Create r_tmp directory
if (!dir.exists("r_tmp")) {
  dir.create("r_tmp")
}

test_that("Bootstrapped (Continuous) Meta-Analysis Runs", {
  set.global.conf.level(95)
  results <- bootstrap.continuous(fname, tmp_obj, params)
