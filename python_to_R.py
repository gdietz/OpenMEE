...
def run_histogram(
    ...
    # get data
    studies = model.get_studies_in_current_order()
    data = [study.get_var(var) for study in studies if study.get_var(
        var) not in [None, ""]]
...
def prepare_funnel_params_for_R(params):
    ...
    try:
        ...
        pass
    try:
        paramsR['addtau2'] = "TRUE" if params['addtau2'] is True else "FALSE"
    except KeyError:
        pass
    try:
...
def joiner(alist, sep=", ", line_length=80):
    '''
    same as str.join except will insert a newline every line_length
    characters.
    This is to address a stupid issue in R, and by the transitive property,
    rpy2, wherein if the input to the terminal is very very long, it just
    won't work without a newline here and there (assumed due to buffering
    issues)
    '''
    ...
    for i, x in enumerate(alist):
        ...
        if current_line_length > 0 and potential_line_length >= line_length:
            result_string += "\n"
            current_line_length = 0
...
def transform_effect_size(metric, source_data, direction, conf_level):
    ...
    if direction == TRANS_TO_RAW:
        ...
        r_str = "trans.to.raw(metric='%s', source.data=%s, conf.level=%f)" % (
            metric_str, source_dataf.r_repr(), conf_level)
        ...
        source_dataf = ro.DataFrame(d)
        r_str = "raw.to.trans(metric='%s', source.data=%s, conf.level=%f)" % (
            metric_str, source_dataf.r_repr(), conf_level)
    ...
    if direction == RAW_TO_TRANS:
        result = {TRANS_EFFECT: result['yi'], TRANS_VAR: result['vi']}
    elif direction == TRANS_TO_RAW:
        result = {
            RAW_EFFECT: result['yi'],
            RAW_LOWER: result['lb'],
            RAW_UPPER: result['ub'],
        }
...
def get_available_methods(
    ...
    is_special = lambda f: any([f.endswith(ending)
    ...
    if for_data_type is not None:
        all_methods = [
            ...
            if method.startswith(for_data_type)
        ]

    # now, if a data object handle was provided, check which methods are
    # feasible
    if data_obj_name is not None:
        ...
        for method in all_methods:
            ...
            if is_feas_f in method_list:
                ...
                is_feasible = exR.execute_in_R(
                    "%s(%s, '%s')" % (is_feas_f, data_obj_name, metric))[0]
                if funnel_mode:
                    is_feas_4_funnel_f = "%s.is.feasible.for.funnel" % method
                    is_feasible_for_funnel = exR.execute_in_R(
                        "%s()" % (is_feas_4_funnel_f))[0]
            ...
            if is_feasible:
                ...
                if pretty_names_f in method_list:
                    pretty_name = exR.execute_in_R(
                        "%s()$pretty.name" % pretty_names_f)[0]
...
def get_params(method_name):
    ...
    names_and_descriptions = get_pretty_names_and_descriptions_for_params(
        method_name,
        param_list,
    )
...
def get_pretty_names_and_descriptions_for_params(method_name, param_list):
    ...
    if pretty_names_f in method_list:
        ...
        pretty_names_and_descriptions = exR.execute_in_R(
            "%s()" % pretty_names_f)
    ...
    for text_n, text in result.items():
        ...
        if text_n == "images":
            ...
            references_list.append(
                'metafor: Viechtbauer, Wolfgang. "Conducting meta-analyses \
in R with the metafor package." Journal of 36 (2010).')
            references_list.append(
                'OpenMetaAnalyst: Wallace, Byron C., Issa J. Dahabreh, \
Thomas A. Trikalinos, Joseph Lau, Paul Trow, and Christopher H. Schmid. \
"Closing the Gap between Methodologists and End-Users: R as a Computational \
Back-End." Journal of Statistical Software 49 (2012): 5."')
            ...
            # This is the data that was passed to the analysis function
            pass
        elif text_n == "input_params":
            # these are the input parameters that were passed to the analysis
            # function
            pass
            ...
            # 'gui.ignore' is a directive to the gui to ignore the data
            pass
        elif text_n == "weights":
...
def extract_additional_values(res, res_info, sublist_prefix="__"):
    ...
    # treated as the beginning of a sublist in the results_window (a tree
    ...
    if _is_grouped_result(res_info):
        ...
        for i, subgroup_name in enumerate(subgroup_header_names):
            ...
            grouped_values_and_value_info[
                subgroup_name] = extract_values_for_results_data_single_result(
                    sub_res, sub_res_info,
                )
        ...
        additional_values_dict = extract_values_for_results_data_single_result(
            res, res_info)
...
def _is_grouped_result(res_info):
    ...
    for key in res_info_keys:
        ...
        type_and_description_present = type_and_description_keys_in_sublist(
            sublist)
        type_and_description_present_list.append(type_and_description_present)

    if all(type_and_description_present_list):
        # all the sublists have 'type' and 'description'
...
def extract_values_for_results_data_single_result(res, res_info):
    ...
    for key in value_names:
        value_info[key] = {
            'type': res_info.rx2(key).rx2('type')[0],
    ...
    for k, v in values_for_csv.iteritems():
        if value_info[k]['type'] != 'vector':
            ...
            try:
                ...
                # for cases of TypeError: 'rpy2.rinterface.RNULLType' object is
                # not iterable
                if str(v) == "NULL":
    ...
    # Condense value_info and values in to one dictionary to simply the
    # structures and logic e.g.
    # toReturn = {'b':{'type':'vector','description':"prettyvector",'value': 5}
...
def make_weights_str(results):
    '''
    Make a string representing the weights due to each study in the meta
    analysis

    results must contain the following keys:
        weights
        input_data
        input_params
    '''

    assert "weights" in results, "weights must be in results"
    assert "input_data" in results, "input_data must be in results"
    assert "input_params" in results, "input_params must be in results"
...
def run_continuous_ma(
    ...
    r_str = "%s<-%s(%s, %s)" % (res_name, function_name,
                                cont_data_name, params_df.r_repr())
...
def run_meta_method(
    ...
    meta_function_name argument.
    '''

    params_df = ro.r['data.frame'](**params)
    r_str = "%s<-%s('%s', %s, %s)" % \
            (res_name, meta_function_name, function_name,
             data_name, params_df.r_repr())
...
def _make_mods_listVector(covariates, interactions):
    ...
    def make_mod_vector(covlist):
        ...
        return ro.StrVector(
            ["%s" % cov.get_label_R_compatible() for cov in covlist],
        )
    ...
    def make_interactions_listVector(interactions):
        ...
        for interaction in interactions:
            interaction_str = '"%s"=%s' % (
                interaction.r_colon_name(), interaction.r_vector_of_covs())
...
def get_btt_indices(data, mods, choice, choice_type):
    ...
    # choice is a covariate or interaction inside one of the lists of inside
    # of mods
    if choice is None:
        return []
    # Choice as it exists in mods
    choice_R = choice.get_label_R_compatible(
    ) if choice_type == "covariate" else choice.r_colon_name()

    #########
    # Take mods and for each element (numeric, categorical, interactions) build
    # an ordered dictionary mapping each cov/interacton to # of generated
    # coefficients
    ...
    for cat_mod in categorical_mods:
        ...
        n_coeffs_odict["categorical"][cat_mod] = len(
            levels) - 1  # minus 1 for the intercept term
    ...
    for inter_name in interaction_names:
        ...
        n_coeffs_odict["interactions"][
            inter_name] = n_coeffs(cov1) * n_coeffs(cov2)
    # counting complete
    ############

    # Find choice_R in n_coeffs_odict, summing along the way to find the
    # indices of interest
    start_less_one = 1  # intercept at index 1
    for main_category in ["numeric", "categorical", "interactions"]:
        for coeff_name, num_coeff in n_coeffs_odict[main_category].items():
            if coeff_name == choice_R:
                start_idx = start_less_one + 1
                last_idx = start_idx + num_coeff - 1
                return range(start_idx, last_idx + 1)
            else:
                start_less_one += num_coeff
...
def run_gmeta_regression_bootstrapped_cond_means(
    ...
    r_str = '''
    {results} <- g.bootstrap.meta.regression.cond.means(
        data={data},
        mods={mods},
        method=\"{method}\",
        level={level},
    ...
    if (selected_cov, covs_to_values) != (None, None):
        ...
        r_str = "%s<- meta.regression(%s, %s, %s)" % \
            (results_name, data_name, str(params_df.r_repr()),
             ro.DataFrame(meta_reg_d).r_repr())
    else:
...
def run_bootstrap_meta_regression(
    ...
    if (selected_cov, covs_to_values) != (None, None):
        ...
        '''.format(
            ...
            fname="'boeuf'",
            omdata=data_name,
            params=str(params_df.r_repr()),
            cond_means_data=str(ro.DataFrame(meta_reg_d).r_repr()),
        )
    else:
        r_str = "%s<-bootstrap(%s, %s, %s)" % (
            results_name,
            "'boeuf'",
            data_name,
            str(params_df.r_repr()),
        )
...
def get_method_description(method_name):
    ...
    if pretty_names_f in method_list:
        try:
            description = exR.execute_in_R(
                "%s()$description" % pretty_names_f)[0]
        except:
            pass
    return description
...
class R_parse_tools:
    ...
    def rlist_to_pydict(named_r_list):
        ...
        # simple: if True, the data doesn't get messed with at all, otherwise
        # singletons are converted to scalars
    ...
    def recursioner(data):
        ...
        if R_parse_tools.haskeys(data):  # can be converted to dictionary
            ...
            # convert NA to None
            return R_parse_tools._convert_NA_to_None(data)
    ...
    def _singleton_list_to_scalar(singleton_list):
        '''
        Takes in a singleton R list and returns a scalar value and converts
        'NA' to None
        '''

        if len(singleton_list) > 1:
            raise ValueError(
                "Expected a singleton list but this list has multiple entries")

        # special case of a factor ve
        if type(singleton_list) == rpy2.robjects.vectors.FactorVector:
            r_str = "as.character(%s)" % singleton_list.r_repr()
            return exR.execute_in_R(r_str)[0]
...
def update_plot_params(
    ...
    for param_name in plot_params:
        r_str = "%s$%s <- tmp.params$%s" % (plot_params_name,
...
def write_out_plot_data(params_out_path, plot_data_name="plot.data"):
    exR.execute_in_R("save.plot.data(%s, '%s')" %
                     (plot_data_name, params_out_path))
...
def list_of_cov_value_objects_str(studies, cov_list=[], cov_to_ref_var={}):
    ...
    for cov in cov_list:
        ...
        r_cov_str.append(_gen_cov_vals_obj_str(
            cov, studies, ref_var_as_string))
...
def _gen_cov_vals_obj_str(cov, studies, ref_var=None):
    ...
    values_str, cov_vals = cov_to_str(
        cov, studies, named_list=False, return_cov_vals=True)
    if ref_var is None:  # just take the first value if unspecified
        # arbitrary, get rid of single quotes
        ref_var = cov_vals[0].replace("'", "")
    ...
    new(
        ...
        ref.var='{ref_var}')
    '''.format(
        cov_name=cov.get_label(),
        cov_values=values_str,
        cov_type=COVARIATE_TYPE_TO_OMA_STR_DICT[cov.get_type()],
        ref_var=ref_var
    )
    return r_str
...
def cov_to_str(cov, studies, named_list=True, return_cov_vals=False):
    '''
    The string is constructed so that the covariate values are in the same
    order as the studies list.
    '''
    cov_str = None
    if named_list:
    ...
    cov_values = [convert_cov_value(cov.get_type(), value)
...
def run_model_building(
    ...
    # Mods is a Listvector (see description in _make_mods_listVector)
    mods_full = _make_mods_listVector(
        full_model_info['covariates'], full_model_info['interactions'])
    mods_reduced = _make_mods_listVector(
        reduced_model_info['covariates'], reduced_model_info['interactions'])

    r_str = '''
    {results} <- model.building(
        data={data},
        full.mods={full_mods},
        reduced.mods={reduced_mods},
        method=\"{method}\",
        level={level},
        digits={digits})
    '''.format(
        results=results_name,
        data=data_name,
        full_mods=mods_full.r_repr(),
        reduced_mods=mods_reduced.r_repr(),
        method=random_effects_method,
        level=conf_level,
        digits=digits)
...
def run_phylo_ma(
    ...
    {results} <- phylo.meta.analysis(
        ...
        lambda={lambda_},
        alpha={alpha},
        include.species={include_species},
...
def covariates_to_dataframe(model, studies, covariates):
    # This is for imputation using mice. We create an R dataframe of the
    # portion of the dataset of interest suitable for sending on to mice.
    ...
    def var_values(cov, NA_type):
        ...
        # convert None to R NA of the appropriate variety
        values = [None_to_NA(x, NA_type) for x in values]
        return values

    # Covariates sorted into 'int', 'continuous', and 'factor' categories
    sorted_covariates = sort_covariates_by_type(
        covariates, sort_method=INT_CONTINUOUS_FACTOR)
    ...
    int_covs = dict([(cov, cov_to_IntVector(cov))
                     for cov in sorted_covariates['int']])
    cont_covs = dict([(cov, cov_to_FloatVector(cov))
                      for cov in sorted_covariates['continuous']])
    cat_covs = dict([(cov, cov_to_FactorVector(cov))
                     for cov in sorted_covariates['factor']])
    ...
    ordered_init_dict = rlc.OrdDict(
        [(cov.get_label(), covs_to_Rvectors[cov]) for cov in covariates])
...
def dataframe_to_ordered_dict(dataframe, covariates):
    ...
    def vector_to_values(vector):
        ...
        if type(vector) in [ro.IntVector, ro.FloatVector, ro.StrVector]:
            ...
            raise Exception("This type is unrecognizes: %s" %
                            str(type(vector)))
...
def imputation_dataframes_to_pylist_of_ordered_dicts(
    ...
    imputation_choices = [dataframe_to_ordered_dict(
        df, covariates) for df in imputations_list]
...
def impute(
    ...
    # Also store the df as a python ordered dictionary for the purposes of
    # seeing which values were None
...
def rstr_for_rfn(fname, **kargs):
    ...
    keyword_argument_substrs = ["{k}={v}".format(
        k=key, v=val) for key, val in kargs.iteritems()]
...
def make_imputed_datasets(
    ...
    r_str = "original.dataset <- {source_dataset}".format(
        source_dataset=original_dataset.r_repr()
    )
    exR.execute_in_R(r_str)

    r_str = "imputations <- {imputations}".format(
        imputations=imputations.r_repr()
    )
    exR.execute_in_R(r_str)

    r_str = '''
    {imputed_datasets_name} <- combine.imputations.with.dataset(
        original.dataset,
        imputations)
    '''.format(
...
def run_multiple_imputation_meta_analysis(
    ...
    r_str = "params <- {params}".format(params=params.r_repr())
    exR.execute_in_R(r_str)

    # Make mods is a Listvector and place it in the R workspace
    mods = _make_mods_listVector(covariates, interactions)
    r_str = "mods <- {mods}".format(mods=mods.r_repr())
    exR.execute_in_R(r_str)

    r_str = '''
    {results_name} <- multiply.imputed.meta.analysis(
        imputed.datasets={imputed_datasets_name},
        mods=mods,
        rma.args=params)
    '''.format(
        results_name=results_name,
...
def regenerate_forest_plot_of_coefficients(
    ...
    r_str = "regenerate.coeff.forest.plot(input.df, \"%s\", \"%s\")" % (
        file_path_wo_extension, img_fmt)
    exR.execute_in_R(r_str)
...
def get_R_libpaths():
    '''
    Returns the libpaths that R looks at, sanity check to make sure it sees the
    right paths
    '''
...
def get_data_exploration_analyses():
    ...
    analyses_info = get_nested_rlist_as_pydict(
        'get.data.exploration.analyses()')
    analyses_info = singletonlists_to_scalars(analyses_info)
    return analyses_info

if __name__ == '__main__':
    # app = QApplication(sys.argv)
    # form = BinaryCalculator(
    #     conf_level=DEFAULT_CONFIDENCE_LEVEL,
    #     debug=True,
    #     digits=3)
    rloader = RlibLoader()
...
def dynamic_save_plot(rfunction, plot_data_path, file_path, format):
    ...
    file_path location.
        rfunction - string representing r save function e.g.
            'weighted.histogram.plot'. This R function is expected to take the
            following arguments as elements of an R list:
                plot.data - which is the name of the object that we reload on
                    the R side
                base.path (path w/o extension to desired save location e.g.
                    \Users\yourusername\myimage)
    ...
    rarguments = "list(plot.data=%s, base.path='%s', image.format='%s')" % (
